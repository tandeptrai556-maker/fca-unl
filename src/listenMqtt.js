...
function buildStream(options, WebSocket, Proxy) {
    ...
    WebSocket.onerror = (err) => {
        console.log('Error: WebSocket.onerror', err); // Thêm dòng này
        clearTimers();
        Stream.destroy(err);
    };
...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
    ...
    global.mqttClient.on('error', (err) => {
        console.log('Error: mqttClient.on(error)', err); // Thêm dòng này
    ...
    global.mqttClient.on('message', (topic, message, _packet) => {
        try {
            ...
            console.log('Error: mqttClient.on(message)', err); // Thêm dòng này
        }
    });
    ...
    const resolveAttachmentUrl = (i) => {
      if (!delta.attachments || i === delta.attachments.length || utils.getType(delta.attachments) !== 'Array') {
        ...
        try {
          ...
          console.log('Error: parseDelta.formatDeltaMessage', err); // Thêm dòng này
        ...
        if (attachment.mercury.attach_type === 'photo') {
          api.resolvePhotoUrl(attachment.fbid, (err, url) => {
            if (err) {
                console.log('Error: parseDelta.resolvePhotoUrl', err); // Thêm dòng này
            }
        ...
        if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
            ...
            attachments: ( delta.deltaMessageReply.message.attachments || [] )
              ...
              .map((att) => {
                ...
                try {
                  ...
                  console.log('Error: parseDelta._formatAttachment', ex); // Thêm dòng này
                  x = att;
                  x.error = ex;
                  x.type = 'unknown';
                }
          ...
          if (delta.deltaMessageReply.repliedToMessage) {
            ...
            return defaultFuncs
              ...
              .catch((err) => {
                  console.log('Error: parseDelta.messageReply.forcedFetch', err); // Thêm dòng này
  ...
  switch (delta.class) {
    case 'ReadReceipt': {
      ...
      try {
        ...
        console.log('Error: parseDelta.formatDeltaReadReceipt', err); // Thêm dòng này
        return log.error('Lỗi Nhẹ', err);
      }
      globalCallback(null, fmtMsg);
      break;
      ...
      switch (delta.type) {
        ...
        case 'participant_joined_group_call': {
          ...
          try {
            ...
            console.log(delta);
            console.log('Error: parseDelta.formatDeltaEvent', err); // Thêm dòng này
            return log.error('Lỗi Nhẹ', err);
          }
          globalCallback(null, fmtMsg);
          break;
    ...
    case 'ForcedFetch': {
      ...
      if (mid && tid) {
        ...
        defaultFuncs
          ...
          .then((resData) => {
            ...
            if (utils.getType(fetchData) === 'Object') {
              ...
              switch (fetchData.__typename) {
                ...
                default:
                  console.log('Error: parseDelta.ForcedFetch', fetchData); // Thêm dòng này
          ...
          .catch((err) => {
              console.log('Error: parseDelta.ForcedFetch.catch', err); // Thêm dòng này
              log.error('forcedFetch', err)
          });
      }
      break;
    ...
    case 'NewMessage': {
      ...
      if (delta.attachments?.length === 1 && hasLiveLocation(delta)) {
        ...
        try {
          ...
          console.log('Error: parseDelta.NewMessage.hasLiveLocation', err); // Thêm dòng này
          console.log(delta);
...
function markDelivery(ctx, api, threadID, messageID) {
  if (threadID && messageID) {
    api.markAsDelivered(threadID, messageID, (err) => {
      if (err) {
        console.log('Error: markAsDelivered', err); // Thêm dòng này
        log.error('markAsDelivered', err);
      } else {
        if (ctx.globalOptions.autoMarkRead) {
          api.markAsRead(threadID, (err) => {
            if (err) {
                console.log('Error: markAsRead', err); // Thêm dòng này
                log.error('markAsDelivered', err);
            }
          });
        }
...
function getRespData(Type, payload) {
  try {
    ...
    console.log('Error: getRespData', e); // Thêm dòng này
    return null;
  }
}
...
function LogUptime() {
  ...
  try {
    ...
    console.log('Error: LogUptime', err); // Thêm dòng này
  }
}
...
if (global.Fca.Require.FastConfig.AntiGetInfo.AntiGetThreadInfo) {
  setInterval(() => {
    try {
      ...
      console.log('Error: AntiGetThreadInfo Interval', e); // Thêm dòng này
    }
  }, 30 * 1000);
}
...
module.exports = function(defaultFuncs, api, ctx) {
    ...
    getSeqID = function() {
        console.log("[LOG] Bắt đầu gọi getSeqID để lấy sequence ID...");
        ctx.t_mqttCalled = false;
        defaultFuncs
            .post("https://www.facebook.com/api/graphqlbatch/", ctx.jar, form)
            .then(utils.parseAndCheckLogin(ctx, defaultFuncs))
            .then((resData) => {
                console.log("[LOG] Nhận được phản hồi từ graphqlbatch.");

                if (utils.getType(resData) != "Array") {
                    console.error("[LOG] Phản hồi không phải là một Array. Có thể đã mất đăng nhập.");
                ...
                console.log("[LOG] Dữ liệu phản hồi hợp lệ, đang xử lý...");
                if (resData && resData[resData.length - 1].error_results > 0) {
                    console.error("[LOG] Phản hồi GraphQL có lỗi:", resData[0].o0.errors);
                    throw resData[0].o0.errors;
                }
                if (resData[resData.length - 1].successful_results === 0) {
                     console.error("[LOG] Phản hồi GraphQL không có kết quả thành công.");
                ...
                if (sync_sequence_id) {
                    console.log(`[LOG] Lấy sequence ID thành công: ${sync_sequence_id}`);
                    ctx.lastSeqId = sync_sequence_id;
                    console.log("[LOG] Bắt đầu khởi chạy listenMqtt...");
                    listenMqtt(defaultFuncs, api, ctx, globalCallback);
                } else {
                     console.error("[LOG] Không tìm thấy sync_sequence_id trong phản hồi.");
                    throw { error: "getSeqId: no sync_sequence_id found.", res: resData };
                }
            })
            .catch((err) => {
                console.error("[LOG] LỖI trong chuỗi promise của getSeqID:", err);
                log.error("getSeqId", err);
                if (utils.getType(err) == "Object" && err.error === "getSeqId: no sync_sequence_id found.") {
                    console.log("[LOG] Đánh dấu trạng thái đã đăng xuất.");
                    ctx.loggedIn = false;
                }
    ...
    return function(callback) {
        console.log("[LOG] Trình lắng nghe (listener) đang được khởi tạo...");
        class MessageEmitter extends EventEmitter {
            stopListening(callback) {
                callback = callback || (() => {});
                console.log("[LOG] Đang dừng trình lắng nghe (stopListening)...");
                ...
                if (ctx.mqttClient) {
                    ...
                    ctx.mqttClient.end(false, function(...data) {
                        console.log("[LOG] MQTT client đã kết thúc.");
        ...
        globalCallback = (callback || function(error, message) {
            if (error) {
                console.error('[LOG] Lỗi được đẩy về globalCallback:', error);
                return msgEmitter.emit("error", error);
            }
            // console.log("[LOG] Nhận được message mới, đang đẩy đi...");
            msgEmitter.emit("message", message);
        });

        console.log("[LOG] Reset trạng thái cho lần nghe mới.");
        ...
        if (!ctx.firstListen || !ctx.lastSeqId) {
            console.log("[LOG] Lần nghe đầu tiên hoặc không có lastSeqId, đang gọi getSeqID...");
            getSeqID();
        } else {
            console.log("[LOG] Đã có lastSeqId, gọi thẳng listenMqtt...");
            listenMqtt(defaultFuncs, api, ctx, globalCallback);
        }
